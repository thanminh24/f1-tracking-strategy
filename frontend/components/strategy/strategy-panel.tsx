...
export function StrategyPanel({ sessionKey, asrAvailable = false }: Props) {
  const state = useRaceStateStore((s) => s.state);
  const source = useRaceStateStore((s) => s.source);
  const showComms = hasLiveComms(source);
  const sharedFocusCar = useRaceStateStore((s) => s.focusedCarId);
  const setSharedFocusCar = useRaceStateStore((s) => s.setFocusedCarId);
  const cars = state?.cars.filter((c) => c.status !== "out") ?? [];
  const [localFocusCar, setLocalFocusCar] = useState<string | null>(null);
  const effectiveCar = sharedFocusCar ?? localFocusCar ?? cars[0]?.car_id ?? null;
  ...
  return (
    <div className="flex flex-col gap-3 p-3 h-full overflow-y-auto scrollbar-thin">
      ...
      {cars.length > 0 && (
        <select
          ...
          onChange={(e) => {
            ...
            setLocalFocusCar(next);
            setSharedFocusCar(next);
          }}
      ...
      {/* Team radio timeline — live/fixture only */}
      {showComms ? (
        <details className="border border-f1-border rounded overflow-hidden">
          ...
          <TeamRadioTimeline sessionKey={sessionKey} asrAvailable={asrAvailable} />
        </details>
      ) : null}
    </div>
  );
